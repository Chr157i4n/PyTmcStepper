...
class Tmc2209(Tmc220x, StallGuard):
    ...
    def do_homing(
        ...
        if self.tmc_com is None:
            self.tmc_logger.log(
                "do_homing only works with VActual register control via COM",
                Loglevel.ERROR,
            )
            return False
        ...
        if speed_rpm is None:
            speed_rpm = (
                tmc_math.steps_to_rps(
                    self.tmc_mc.max_speed_homing, self.tmc_mc.steps_per_rev
                )
                * 60
            )
    ...
    def do_homing2(self, revolutions=10, threshold=100):
        ...
        if not isinstance(self.tmc_mc, TmcMotionControlStepDir):
            self.tmc_logger.log(
                "do_homing2 only works with STEP/DIR Control", Loglevel.ERROR
            )
            return
        sgresults = []
        ...
        coolstep_thres = tmc_math.steps_to_tstep(
            self.tmc_mc.max_speed_homing * 0.5, self.tmc_mc.mres
        )
